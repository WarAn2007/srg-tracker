...
**Version:** V2.4 (training, evaluation, and model-deployment project)
...
**SRG Tracker - Student Ranking & GPA Tracker.** The implemented scope is GPA tracking and early warning; individual ranking is explicitly out of scope.

## 3. Dataset source

The project includes a deterministic synthetic educational dataset generated with seed `20260730`. It contains 450 student-disjoint course attempts, each with weekly records for weeks 1-14. The feature ideas and correlation exploration were inspired by OULAD, but no real OULAD learner records are included or predicted.

You can also import a labelled external CSV. Put private CSV files in `data/incoming/` and run `python -m src.data_import`. The importer validates weekly histories and prevents a student from appearing in more than one train/validation/test split. Do not commit real student data.
...
The training default above remains MLP with Adam. A separately trained linear Ridge GPA model is also available for deployment when a smaller, simpler model is preferred. Its validation RMSE is 0.7750 (rank 6); this is worse than the configured MLP result and should be an explicit trade-off, not a claim of better accuracy. The current `last-change-1` web-project snapshot uses this linear GPA artifact. See [docs/training_and_deployment.md](docs/training_and_deployment.md) for retraining and deployment commands.
...
Generate the deterministic data, train all configured tasks, freeze the selection, and run tests:
...
To train only the GPA task, use:

```powershell
.\.venv\Scripts\python.exe -m src.train --tasks gpa
```

To select the linear Ridge GPA candidate instead of the default MLP, use:

```powershell
.\.venv\Scripts\python.exe -m src.train --tasks gpa --selected-model gpa=linear
```

For a full CSV import, use `python -m src.data_import --input data/incoming/records.csv --tasks gpa`. The complete schema, split workflow, selective-training commands, and web deployment instructions are in [docs/training_and_deployment.md](docs/training_and_deployment.md).
...
## 9. Demo and inference instructions

Ready V2.4 artifacts are included in `models/`. After installing dependencies,
...
- The training project has no production authentication, data storage, or real-LMS integration. The companion web project is still a local demo, not a production deployment.
...
```text
data/incoming/        ignored location for private external CSV files
data/templates/       external-dataset column template
data/processed/       generated or imported student-disjoint CSV splits
data/metadata/        dataset card
docs/                 input contract plus training and deployment guide
