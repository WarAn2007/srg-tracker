...
function scoreError(label: string, field: "assignmentScore" | "quizScore", week: WeekDraft, weeks: WeekDraft[]) {
  const basic = numericError(label, week[field], 0, 100, true);
  if (basic || !week[field].trim()) return basic;
  const alreadyRecorded = weeks.some((other) => other.week < week.week && (other.week < 7) === (week.week < 7) && other[field].trim() !== "");
  return alreadyRecorded ? `${label} may be entered only once ${week.week < 7 ? "before" : "after"} the midterm.` : "";
}

function validateWeek(week: WeekDraft, weeks: WeekDraft[]) {
  return numericError("Weekly grade", week.weeklyGrade, 0, 100)
    || numericError("Attendance", week.attendance, 0, 100)
    || scoreError("Assignment score", "assignmentScore", week, weeks)
    || scoreError("Quiz score", "quizScore", week, weeks)
...
function NumberField({ label, value, onChange, min = 0, max = 100, step = "0.1", optional = false, hint, error = "" }: { label: string; value: string; onChange: (value: string) => void; min?: number; max?: number; step?: string; optional?: boolean; hint?: string; error?: string }) {
  const errorId = `${label.toLowerCase().replaceAll(" ", "-")}-error`;
  return <label className={`field ${error ? "field-invalid" : ""}`}><span>{label}{optional && <em>optional</em>}</span><input type="number" inputMode="decimal" min={min} max={max} step={step} value={value} onChange={(event) => onChange(event.target.value)} placeholder={`${min}-${max}`} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} />{error ? <small id={errorId} className="field-error">{error}</small> : hint && <small>{hint}</small>}</label>;
}
...
export default function App() {
  ...
  const preAssessment = useMemo(() => ({ assignment: weeks.some((week) => week.week < 7 && week.assignmentScore !== ""), quiz: weeks.some((week) => week.week < 7 && week.quizScore !== "") }), [weeks]);
  const gradeError = currentWeek.weeklyGrade.trim() ? numericError("Weekly grade", currentWeek.weeklyGrade, 0, 100) : "";
  const attendanceError = currentWeek.attendance.trim() ? numericError("Attendance", currentWeek.attendance, 0, 100) : "";
  const assignmentError = scoreError("Assignment score", "assignmentScore", currentWeek, weeks);
  const quizError = scoreError("Quiz score", "quizScore", currentWeek, weeks);
  const delayError = currentWeek.submissionDelayDays.trim() ? numericError("Submission delay", currentWeek.submissionDelayDays, 0, 30) : "";
  const correctionsError = currentWeek.correctionsCount.trim() ? numericError("Corrections", currentWeek.correctionsCount, 0, 100) : "";
  const midtermError = activeWeek === 7 && currentWeek.midtermScore.trim() ? numericError("Midterm score", currentWeek.midtermScore, 0, 100) : "";
  const semesterError = identity.semester.trim() ? (!Number.isInteger(Number(identity.semester)) ? "Semester must be a whole number from 1 to 20." : numericError("Semester", identity.semester, 1, 20)) : "";

  useEffect(() => { saveSettings(settings); }, [settings]);
  const refreshHealth = () => checkHealth().then(setHealth).catch(() => setHealth({ status: "offline", models_ready: false, registry_error: "FastAPI is not reachable.", version: "4.0", models: [] }));
  ...
  const registerIdentity = (event: React.FormEvent) => {
    ...
    if (semesterError || !identity.semester.trim()) return setToast(semesterError || "Semester is required.");
    setStage("history");
  };
  const addWeek = () => {
    if (activeWeek !== weeks.length) return setToast(`Open week ${weeks.length} before adding the next week.`);
    const error = validateWeek(currentWeek, weeks);
    if (error) return setToast(`Week ${activeWeek}: ${error}`);
    if (activeWeek === 6 && (!preAssessment.assignment || !preAssessment.quiz)) return setToast("Record one assignment and one quiz before week 7.");
  ...
  const openReview = () => {
    for (const week of weeks) {
      const error = validateWeek(week, weeks);
      if (error) { setActiveWeek(week.week); setToast(`Week ${week.week}: ${error}`); return; }
    }
    if (weeks.length >= 7 && (!preAssessment.assignment || !preAssessment.quiz)) return setToast("Record one assignment and one quiz before week 7.");
    setStage("review");
  };
  const predict = async () => {
    if (!health?.models_ready) return setToast(health?.registry_error ?? "Packaged models are not ready.");
    setStage("predicting");
    try { setPrediction(await requestPrediction(identity, weeks)); setStage("results"); }
    catch (error) { setStage("review"); setToast(error instanceof Error ? error.message : "Prediction failed."); }
  };
  const reset = () => { setIdentity(EMPTY_IDENTITY); setWeeks([emptyWeek(1)]); setActiveWeek(1); setPrediction(null); setStage("identity"); };
  const healthLabel = health?.models_ready ? "3 models ready" : health ? "Model attention required" : "Checking models";

  return <main className={`app-shell stage-${stage}`} style={{ "--accent": settings.colors[1], "--highlight": settings.colors[2] } as React.CSSProperties}>
    <Lightfall colors={settings.colors} backgroundColor={settings.backgroundColor} paused={pausedBackground} /><div className="field-shade" />
    <header className="topbar"><button className="brand" onClick={() => setStage("intro")} aria-label="Go to SRG-Tracker home"><span className="brand-glyph">S</span><span><b>SRG-Tracker</b><small>Student signal observatory</small></span><em>4.0</em></button><nav aria-label="Application utilities"><button onClick={() => setDialog("history")}>History</button><button onClick={() => setDialog("credits")}>Credits</button><button onClick={() => setDialog("settings")}>Settings</button></nav><div className={`health-chip ${health?.models_ready ? "ready" : "warning"}`}><i />{healthLabel}</div></header>
    {stage !== "intro" && <StepRail stage={stage} />}
    {stage === "intro" && <section className="hero"><div className="hero-copy"><p className="eyebrow">Observed history → advisory signals</p><h1>See the course<br />before it <span>ends.</span></h1><p className="hero-lead">Enter consecutive weekly observations and receive a careful estimate of final GPA, course outcome, and learning pace.</p><div className="hero-actions"><button className="primary-button" onClick={() => setStage("identity")}>Start assessment <span>→</span></button><button className="quiet-button" onClick={() => setDialog("credits")}>How this works</button></div><p className="responsibility-line"><i />Educational demonstration · Synthetic-data models · Human review required</p></div><aside className="signal-card"><div className="signal-card-header"><span>Active model registry</span><b>{health?.models_ready ? "READY" : "CHECKING"}</b></div>{health?.models.map((model) => <article key={model.task}><span>{model.task === "gpa" ? "Final GPA" : model.task === "outcome" ? "Course outcome" : "Learning pace"}</span><strong>{model.model_type}</strong><small>{model.validation_metric.replace("_", " ")} {model.validation_value.toFixed(3)} · rank {model.rank}</small></article>)}{health && !health.models_ready && <p className="registry-error">{health.registry_error}</p>}<button onClick={() => setDialog("settings")}>View model information</button></aside></section>}
    {stage === "identity" && <section className="workspace"><div className="workspace-heading"><p className="eyebrow">Step 1 of 4</p><h1>Identify the course attempt</h1><p>These labels appear in the final report and encrypted history. They are not model features except course and semester.</p></div><form className="identity-form" onSubmit={registerIdentity}><label className="field wide"><span>Student ID</span><input autoFocus value={identity.attemptId} onChange={(event) => setIdentity({ ...identity, attemptId: event.target.value })} placeholder="S0001-PY101-1" /><small>Use a demo identifier, never real personal data.</small></label><label className="field"><span>Course ID</span><input value={identity.courseId} onChange={(event) => setIdentity({ ...identity, courseId: event.target.value })} placeholder="PY101" /></label><label className={`field ${semesterError ? "field-invalid" : ""}`}><span>Semester</span><input type="number" min="1" max="20" step="1" value={identity.semester} onChange={(event) => setIdentity({ ...identity, semester: event.target.value })} placeholder="1" aria-invalid={Boolean(semesterError)} />{semesterError && <small className="field-error">{semesterError}</small>}</label><div className="form-footer"><button type="button" className="text-button" onClick={() => setStage("intro")}>Back</button><button className="primary-button" type="submit">Continue to weekly history →</button></div></form></section>}
    {stage === "history" && <section className="workspace history-workspace"><div className="workspace-heading compact"><div><p className="eyebrow">Step 2 of 4 · {identity.courseId.toUpperCase()} · semester {identity.semester}</p><h1>Build the observed history</h1></div><div className="week-summary"><strong>{weeks.length}</strong><span>week{weeks.length === 1 ? "" : "s"}<br />observed</span></div></div><nav className="week-tabs" aria-label="Observed weeks">{weeks.map((week) => <button key={week.week} className={week.week === activeWeek ? "active" : ""} aria-current={week.week === activeWeek ? "step" : undefined} onClick={() => setActiveWeek(week.week)}>W{String(week.week).padStart(2, "0")}</button>)}</nav><div className="week-heading"><div><span>Week {activeWeek}</span><h2>{activeWeek < 7 ? "Pre-midterm observation" : activeWeek === 7 ? "Midterm checkpoint" : "Post-midterm observation"}</h2></div><p>One assignment and one quiz may be recorded in each period.</p></div><div className="data-grid"><NumberField label="Weekly grade" value={currentWeek.weeklyGrade} onChange={(value) => updateWeek("weeklyGrade", value)} error={gradeError} /><NumberField label="Attendance" value={currentWeek.attendance} onChange={(value) => updateWeek("attendance", value)} error={attendanceError} /><NumberField label="Assignment score" optional value={currentWeek.assignmentScore} onChange={(value) => updateWeek("assignmentScore", value)} error={assignmentError} hint={activeWeek < 7 ? "One allowed before midterm" : "One allowed after midterm"} /><NumberField label="Quiz score" optional value={currentWeek.quizScore} onChange={(value) => updateWeek("quizScore", value)} error={quizError} hint={activeWeek < 7 ? "One allowed before midterm" : "One allowed after midterm"} /><NumberField label="Submission delay" value={currentWeek.submissionDelayDays} onChange={(value) => updateWeek("submissionDelayDays", value)} min={0} max={30} hint="Days" error={delayError} /><NumberField label="Corrections" value={currentWeek.correctionsCount} onChange={(value) => updateWeek("correctionsCount", value)} step="1" error={correctionsError} />{activeWeek === 7 && <NumberField label="Midterm score" value={currentWeek.midtermScore} onChange={(value) => updateWeek("midtermScore", value)} hint="Reused unchanged after week 7" error={midtermError} />}{activeWeek > 7 && <div className="locked-value"><span>Midterm score</span><strong>{weeks.find((week) => week.week === 7)?.midtermScore || "—"}</strong><small>Locked from week 7</small></div>}</div><div className="form-footer"><button className="secondary-button" onClick={addWeek} disabled={weeks.length >= 14 || activeWeek !== weeks.length}>+ Add next week</button><button className="primary-button" onClick={openReview}>Review input →</button></div></section>}
    ...
    {dialog === "settings" && <SettingsDialog settings={settings} onSettings={setSettings} onClose={() => setDialog(null)} onNotice={setToast} onModelActivated={() => void refreshHealth()} />}
